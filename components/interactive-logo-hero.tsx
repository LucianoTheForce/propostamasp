...
function InteractiveLogo() {
  ...
  useFrame((state, delta) => {
    ...
    // Calcular escala responsiva - bem pequeno agora
    const viewportSize = typeof window !== "undefined" ? Math.min(window.innerWidth, window.innerHeight) : 800
    const baseScale = viewportSize * 0.0015 // Ainda menor
    const responsiveScale = Math.max(baseScale, 1.2) // Base mínima de 1.2x
